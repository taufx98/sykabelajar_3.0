...
import { GraduationCap, ArrowRight, ArrowLeft, Check, User as UserIcon, School, Calendar, Camera } from 'lucide-react';
...
export function RegisterPage() {
  ...
  const [form, setForm] = useState({
    email: '', password: '', confirmPassword: '', role: 'pelajar' as Role,
  ...
  const selectPhoto = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/') || file.size > 5 * 1024 * 1024) {
      toast('Foto harus gambar dan maksimal 5MB.', 'error');
      return;
    }
    setPhotoFile(file);
  };
