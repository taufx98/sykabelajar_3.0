...
export function resetRuntimeCollections() {
  demoUsers.length = 0; demoCompetitions.length = 0; demoDailyTasks.length = 0; demoLeaderboard.length = 0; demoAwards.length = 0;
  demoCertificates.length = 0; demoNotifications.length = 0; demoOrders.length = 0; demoFeed.length = 0;
...
function mapAccountRole(accountType: string | null | undefined): User['role'] {
  if (accountType === 'teacher') return 'guru';
  if (accountType === 'organizer') return 'penyelenggara';
  return 'pelajar';
}

export function mapProfileToUser(profile: any, email = ''): User {
  return {
    id: profile.id, username: profile.username ?? '', email, displayName: profile.full_name ?? profile.username ?? 'Pengguna',
    role: mapAccountRole(profile.account_type), bio: profile.bio ?? undefined, school: profile.institution ?? undefined,
    ...
    birthDate: profile.birth_date ?? undefined, points: 0, rank: 0, joinedAt: profile.created_at, favoriteCategories: [], profilePhoto: profile.avatar_url ?? undefined,
    badges: [], emblems: [], followers: 0, following: 0, verified: profile.status === 'ACTIVE',
  };
}
...
export async function loadUsers() {
  ...
  demoLeaderboard.push(...rows.map((row, index): LeaderboardEntry => {
    const rankStatus = row.rank_change === 'up' || row.rank_change === 'down' ? row.rank_change : 'same';
    const pointChange = Number(row.point_change ?? 0);
    return {
      rank: Number(row.rank ?? index + 1), userId: String(row.user_id), username: String(row.username ?? ''), displayName: String(row.display_name ?? row.username ?? 'Pengguna'),
      profilePhoto: row.avatar_url ? String(row.avatar_url) : undefined, points: Number(row.xp ?? 0), school: row.institution ? String(row.institution) : undefined,
      educationLevel: undefined, emblems: [], change: rankStatus === 'up' ? 1 : rankStatus === 'down' ? -1 : 0, pointChange, rankStatus,
    };
  }));
  demoUsers.push(...demoLeaderboard.map((u): User => ({
    id: u.userId, username: u.username, email: '', displayName: u.displayName, role: 'pelajar', school: u.school, educationLevel: u.educationLevel,
...
export async function loadCompetitions() {
  ...
  demoCompetitions.push(...((data ?? []) as Array<Record<string, unknown>>).map((row): Competition => ({
    ...
    startDate: String(row.starts_at ?? row.registration_starts_at ?? ''), endDate: String(row.ends_at ?? row.announcement_at ?? ''), registrationDeadline: String(row.registration_ends_at ?? ''),
    status: mapCompetitionStatus(String(row.status ?? 'PUBLISHED')), participants: Number(row.participant_count ?? 0), level: 'Nasional', twibbonUrl: '', hasQuestions: false,
...
function normalizeOrderStatus(status: unknown): OrderStatus {
  ...
  if (value === 'PAID') return 'paid'; if (value === 'SHIPPED') return 'shipped'; if (value === 'COMPLETED') return 'completed'; if (value === 'CANCELLED' || value === 'CANCELED') return 'cancelled'; return 'pending';
}
function normalizeOrderCategory(productType: unknown): OrderItem['category'] {
  const value = String(productType ?? '').toLowerCase();
  if (value.includes('medal') || value.includes('medali')) return 'medali'; if (value.includes('emblem')) return 'emblem'; return 'sertifikat';
}
