...
function normalizeOrderCategory(productType: unknown): OrderItem['category'] {
  ...
  if (value.includes('medal') || value.includes('medali')) return 'medali';
  if (value.includes('emblem')) return 'emblem';
  return 'sertifikat';
}
...
export async function loadOrders(userId: string) {
  ...
  if (error) throw error;
  demoOrders.push(...((data ?? []) as Array<Record<string, any>>).map((o): Order => ({
    id: String(o.id), code: String(o.id), payCode: '', userId: String(o.user_id),
    items: (o.order_items ?? []).map((i: any): OrderItem => ({ id: String(i.id), category: normalizeOrderCategory(i.product_type), itemName: String(i.name ?? ''), quantity: Number(i.quantity ?? 1), price: Number(i.unit_price ?? 0) })),
    total: Number(o.total ?? 0), status: normalizeOrderStatus(o.status), address: String(o.address_id ?? ''), createdAt: String(o.created_at), updatedAt: String(o.updated_at),
  })));
}
...
export async function loadFeed() {
  ...
  if (error) throw error;
  demoFeed.push(...((data ?? []) as Array<Record<string, any>>).map((p): FeedPost => ({
    id: String(p.id), userId: String(p.author_user_id ?? p.user_id), competitionId: p.competition_id ?? undefined, competitionSlug: p.competition_slug ?? undefined,
    title: String(p.title ?? ''), body: String(p.body ?? p.content ?? ''), createdAt: String(p.created_at), likes: Number(p.likes_count ?? 0), reposts: Number(p.reposts_count ?? 0),
    image: p.cover_url ?? undefined, type: p.competition_id ? 'competition' : 'post', meta: p.competition_id ? 'Lomba' : 'Post', comments: [],
  })));
}
