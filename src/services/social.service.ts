...
export async function listPublishedPosts(limit = 30): Promise<SocialPost[]> {
  ...
  const [{ data: profiles }, { data: likes }, { data: comments }, { data: session }] = await Promise.all([
  ...
  return rows.map((p) => {
    ...
    return {
      ...
      liked: !!session.session?.user && likeRows.some((l) => String(l.post_id) === id && String(l.user_id) === session.session?.user.id),
...
export async function togglePostLike(postId: string) {
  ...
  const { data: existing, error: lookupError } = await supabase.from('post_likes').select('post_id').eq('post_id', postId).eq('user_id', userData.user.id).maybeSingle();
  if (lookupError) throw lookupError;
  if (existing) {
    const { error } = await supabase.from('post_likes').delete().eq('post_id', postId).eq('user_id', userData.user.id);
    if (error) throw error;
  } else {
    const { error } = await supabase.from('post_likes').insert({ post_id: postId, user_id: userData.user.id });
    if (error) throw error;
  }
}
...
export async function addPostComment(postId: string, body: string) {
  ...
  const { error } = await supabase.from('comments').insert({ post_id: postId, user_id: userData.user.id, body: text });
  if (error) throw error;
}
