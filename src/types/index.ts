...
export type CompetitionCategory =
  | 'mtk' | 'ipa' | 'ips' | 'bindo' | 'bing' | 'seni' | 'olahraga' | 'tech' | 'lingkungan';
...
export interface Submission {
  ...
  competitionId: string;
  userId: string;
  status: 'in-review' | 'scored' | 'rejected';
  score?: number;
  rank?: number;
  submittedAt: string;
...
export type NotificationType =
  | 'competition-start' | 'result-out' | 'registration-approved'
  | 'registration-rejected' | 'order-update' | 'daily-reminder' | 'rank-up' | 'twibbon-verified';
...
export interface FeedPost {
  ...
  type: 'achievement' | 'competition' | 'daily-task' | 'post';
  title: string;
  body: string;
  createdAt: string;
  likes: number;
